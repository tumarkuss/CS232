...
uint32_t slist_length(struct slist *l){           //changed void to int   FINISHED
    ...
    return(l->size); //updated for resubmission
}
...
struct snode *slist_find_at(struct slist *l, int index) {
  ...
  while (find != NULL){
    ...
    else if (0 < index)  //changed for resubmission from 3. Stupid mistake XD
    {
...
void slist_delete_at(struct slist *l, int index) {
    ...
    if(index<0){
  index = l->size + index; //changed for resubmission
  }
  snode_s * node = slist_find_at(l, index);

  slist_delete(l, snode_get_str(node));
} //I choose option of making this function return nothing.
// All bugs fixed. If you're reading this, thank you for being such a wonderful professor!